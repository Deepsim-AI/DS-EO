...
class SessionDispatcher:
    ...
    def spawn_agent(
        ...
        """
        Spawn a new agent session for handoff.

        This method generates the *instruction* for sessions_spawn — in production,
        it would invoke the actual sessions_spawn tool. Here it returns a structured
        result that can be consumed by the dispatcher engine or logged as proof of dispatch.

        Args:
            target_agent_id: Agent to spawn (e.g., "implementer")
        ...
        session_key = f"agent:{target_agent_id}:subagent:{task_id or 'untracked'}" if task_id else None
